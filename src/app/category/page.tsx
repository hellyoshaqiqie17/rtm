...
const DEFAULT_BG = 'https://images.unsplash.com/photo-1594909122845-11baa439b7bf?w=500&auto=format&fit=crop&q=80';

export default function CategoryPage() {
  const { channels, categories, categoryObjects } = useStreamContext();

  const displayCategories = (categoryObjects && categoryObjects.length > 0
    ? categoryObjects
    ...
    <div className="space-y-8 py-6 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto font-sans selection:bg-[#E50914] selection:text-white">
      ...
      {displayCategories.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 font-sans">
          {displayCategories.map((catObj, idx) => {
            const catName = catObj.name;
            const count = channels.filter((c) => (c.category || 'Tanpa Kategori').toLowerCase() === catName.toLowerCase()).length;
            const bg = catObj.image || categoryBgs[catName] || DEFAULT_BG;
